...
def test_align_outer_ffill_keeps_union_shape() -> None:
    ...
    assert out.height == 5
    assert out["timestamp"][0] == datetime(2024, 1, 1, 0, 0, tzinfo=timezone.utc)
    assert out["timestamp"][-1] == datetime(2024, 1, 1, 0, 4, tzinfo=timezone.utc)
...
def test_compute_returns_simple_and_log() -> None:
    ...
    assert simple["value_simple_ret_1"][1] == 0.10
...
def test_align_outer_ffill_with_mixed_tz_duplicates_and_gaps() -> None:
    ...
    a = pl.DataFrame(
        {
            "timestamp": [
                ...
                datetime(2024, 1, 1, 9, 1, tzinfo=timezone.utc),
                datetime(2024, 1, 1, 9, 4, tzinfo=timezone.utc),
            ],
    ...
    assert out["timestamp"][0] == datetime(2024, 1, 1, 9, 0, tzinfo=timezone.utc)
    assert out["timestamp"][-1] == datetime(2024, 1, 1, 9, 4, tzinfo=timezone.utc)
    assert out["value_a"][0] == 1.0
    assert out["value_b"][0] is None
    assert out["value_a"][2] == 2.0
    assert out["value_b"][2] == 101.5
    assert out["value_b"][3] == 102.0
...
def test_align_inner_vs_outer_with_ffill() -> None:
    a = pl.DataFrame(
        {
            "timestamp": [
                datetime(2024, 1, 1, 9, 0, tzinfo=timezone.utc),
                datetime(2024, 1, 1, 9, 2, tzinfo=timezone.utc),
                datetime(2024, 1, 1, 9, 3, tzinfo=timezone.utc),
            ],
            "value_a": [1.0, 2.0, 3.0],
        }
    )
    ...
    assert inner["timestamp"].to_list() == [datetime(2024, 1, 1, 9, 3, tzinfo=timezone.utc)]
